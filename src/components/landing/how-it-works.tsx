...
export function HowItWorks() {
  return (
    <section className="bg-notura-surface px-4 py-20 sm:px-6 lg:px-8">
          ...
          {steps.map((step, i) => (
            ...
            >
              <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-notura-green-light">
                <step.icon className="h-7 w-7 text-notura-green" />
              </div>
              <h3 className="mt-5 font-display text-lg font-semibold text-notura-ink">
                {step.title}
              </h3>
              <p className="mt-2 text-sm leading-relaxed text-notura-muted">
                {step.description}
              </p>
            </motion.div>
          ))}
